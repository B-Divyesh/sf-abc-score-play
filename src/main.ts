...
function setMetadata(title: string, description: string, path: string): void {
  ...
  document.querySelector<HTMLLinkElement>('link[rel="canonical"]')!.href = `https://abc-score-play.sociobot.in${path}`;
}

function editorMarkup(demo: boolean): string {
  return `<section class="work-section" id="workbench" aria-labelledby="workbench-title">
...
function renderRoute(focus = false): void {
  ...
  if (path === '/') {
    ...
    setMetadata('Demo — ABC Score Play', 'Try a sample ABC score, play it, and repeat selected bars.', '/demo');
  ...
  if (focus) {
    ...
    if (announcement) announcement.textContent = document.querySelector('h1')?.textContent ?? document.title;
  } else if (location.hash === '#workbench') {
